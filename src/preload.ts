...
contextBridge.exposeInMainWorld('electronAPI', {
    startHyperateMonitor: (params: StartHyperateMonitorParams) =>
        ipcRenderer.send('start-hyperate-monitor', params),
    ...
    onHeartRateUpdate: (callback: (newHeartRate: number) => () => void) => {
        const callbackFormatted = (_event: any, newHeartRate: any) =>
